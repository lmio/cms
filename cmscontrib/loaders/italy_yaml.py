...
class YamlLoader(ContestLoader, TaskLoader, UserLoader, TeamLoader):
    ...
    def get_contest(self):
        ...
        load(conf, args, "min_submission_interval_grace_period", conv=make_timedelta)
        load(conf, args, "min_user_test_interval", conv=make_timedelta)
        load(conf, args, "min_user_test_interval_grace_period", conv=make_timedelta)
    ...
    def get_task(self, get_statement=True):
        ...
        if os.path.isfile(sharedtest_filename):
            ...
            with open(sharedtest_filename, "rt", encoding="utf-8") as sharedtest_file:
        ...
        if conf.get('output_only', False):
            ...
            task.submission_format = \
                ["%s_%03d.out" % (name, i + 1) for i in range(n_input)]
